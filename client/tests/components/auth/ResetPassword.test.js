...
import { ResetPassword }
  from '../../../src/components/auth/password/ResetPassword.jsx';
...
describe('<ResetPassword/>', () => {
  ...
  it('should set user props', () => {
    ...
    expect(wrapper.find('ResetPasswordForm').props().user)
      .toEqual(correctPassword);
  });
  ...
  it('should call the handleSubmitPasswordReset method', () => {
    ...
    const handleSubmitSpy = jest
      .spyOn(wrapper.instance(), 'handleSubmitPasswordReset');
    wrapper.instance().handleSubmitPasswordReset(event);
    expect(handleSubmitSpy).toHaveBeenCalledTimes(1);
  });
  it(
    'should set the confirmPasswordError state to password do not match',
    () => {
      const wrapper = setup(false);
      const handleSubmitSpy = jest
        .spyOn(wrapper.instance(), 'handleSubmitPasswordReset');
      ...
      wrapper.instance().handleSubmitPasswordReset(event);
      expect(handleSubmitSpy).toHaveBeenCalledTimes(1);
      expect(wrapper.state().confirmPasswordError)
        .toEqual('Passwords do not match');
    }
  );
  it(
    'should set the confirmPasswordError state to password must be at leat 6 characters',
    () => {
      const wrapper = setup(false);
      const handleSubmitSpy = jest
        .spyOn(wrapper.instance(), 'handleSubmitPasswordReset');
  ...
  it(
    'should call the resetPassword and set confirmPasswordError to empty string',
    () => {
      const wrapper = setup(false);
      const handleSubmitSpy = jest
        .spyOn(wrapper.instance(), 'handleSubmitPasswordReset');
